...
<head>
    ...
    <style>
        ...
        .topline{ display:flex; justify-content:space-between; margin-bottom:8px; }
        .small{ font-size:11pt; }
        .title{ text-align:center; font-weight:bold; font-size:16pt; text-transform:uppercase; }
        .sub{ text-align:center; margin-bottom:8px; }
        .hr{ border-bottom:2px solid #000; margin:6px 0 10px; }
        .line{ display:flex; align-items:center; margin-bottom:4px; }
        .idx{ width:18px; }
        .lbl{ min-width:140px; font-weight:bold; }
        .fill{ flex:1; border-bottom:1px dotted #000; min-height:18px; }
        .gap{ width:10px; }
        .squares{ display:inline-flex; gap:2px; }
        .sq{ width:14px; height:18px; border:1px solid #000; display:inline-block; }
        .num{ display:inline-block; min-width:40px; border-bottom:1px dotted #000; text-align:center; padding:0 4px; }
        .checkboxes{ display:flex; gap:16px; }
        .cb{ display:inline-flex; align-items:center; gap:6px; }
        .box{ width:14px; height:14px; border:1px solid #000; display:inline-block; text-align:center; line-height:14px; }
        .checked::after{ content:'✓'; font-weight:bold; font-size:11px; }
        .section-title{ font-weight:bold; margin:8px 0 6px; }
        .textarea{ border:1px solid #000; min-height:80px; padding:6px; margin-bottom:6px; white-space:pre-wrap; }
        .two-col{ display:flex; gap:10px; }
        .col{ flex:1; }
        .vitals{ border:1px solid #000; padding:6px; width:230px; }
        .vrow{ display:flex; margin-bottom:4px; }
        .vrow .vlabel{ width:120px; }
        .sign{ display:flex; justify-content:space-between; margin-top:18px; }
        .sign-box{ width:40%; text-align:center; }
        .sign-line{ height:42px; border-bottom:1px solid #000; margin-bottom:4px; }
    ...
    <?php 
        $printSquares = function($value, $len=2){
            ...
            $out = '';
            for($i=0;$i<$len;$i++){ $out.='<span class="sq"></span>'; }
            return '<span class="squares">'.$out.'</span>';
        };
        $yes = function($cond){ return $cond ? 'checked' : ''; };
...
<body onload="window.print()">
    ...
    <div class="line"><span class="idx">1.</span><span class="lbl">Họ và tên (in hoa):</span><span class="fill"><?= $safe($record['ho_ten'] ?? '') ?></span>
        <span class="gap"></span><span class="lbl" style="min-width:60px;">Tuổi</span><span class="fill" style="max-width:80px;"></span>
    </div>
    ...
    <div class="line"><span class="idx">4.</span><span class="lbl">Dân tộc:</span><span class="fill"><?= $safe($record['dan_toc'] ?? '') ?></span>
        <span class="gap"></span><span class="lbl" style="min-width:100px;">Ngoại kiều:</span><span class="fill"><?= $safe($record['ngoai_kieu'] ?? '') ?></span>
    </div>
    <div class="line"><span class="idx">5.</span><span class="lbl">Nơi làm việc:</span><span class="fill"><?= $safe($record['noi_lam_viec'] ?? '') ?></span></div>
    <div class="line"><span class="idx">6.</span><span class="lbl">Địa chỉ:</span><span class="fill"><?= $safe($record['dia_chi'] ?? '') ?></span></div>
    ...
    <div class="line"><span class="idx">10.</span><span class="lbl">BHYT giá trị đến:</span>
        ...
        <span class="gap" style="width:16px"></span>Số thẻ BHYT: <span class="fill" style="min-width:220px; display:inline-block;"><?= $safe($record['so_the_bhyt'] ?? '') ?></span>
    </div>
    <div class="line"><span class="idx">11.</span><span class="lbl">Điện thoại người báo tin:</span><span class="fill"><?= $safe($record['dien_thoai_bao_tin'] ?? '') ?></span></div>
    ...
    <div class="line"><span class="idx">13.</span><span class="lbl">Chẩn đoán nơi giới thiệu:</span><span class="fill"><?= $safe($record['chan_doan_gioi_thieu'] ?? '') ?></span></div>
    ...
    <div class="two-col">
        <div class="col">
            ...
            <div class="line"><span class="idx">6.</span><span class="lbl">Cho vào điều trị tại khoa:</span><span class="fill"><?= $safe($record['khoa_dieu_tri'] ?? '') ?></span></div>
            <div class="line"><span class="idx">7.</span><span class="lbl">Chú ý:</span><span class="fill"><?= $safe($record['chu_y'] ?? '') ?></span></div>
        </div>
        <div class="vitals">
            <div class="vrow"><span class="vlabel">Mạch:</span><span class="fill" style="border:0;border-bottom:1px dotted #000;"><?= $safe($record['mach'] ?? '') ?></span> lần/phút</div>
            <div class="vrow"><span class="vlabel">Nhiệt độ:</span><span class="fill" style="border:0;border-bottom:1px dotted #000;"><?= $safe($record['nhiet_do'] ?? '') ?></span> °C</div>
            <div class="vrow"><span class="vlabel">Huyết áp:</span><span class="fill" style="border:0;border-bottom:1px dotted #000;"><?= $safe($record['huyet_ap_tam_thu'] ?? '') ?></span>/<span class="fill" style="border:0;border-bottom:1px dotted #000; max-width:60px; display:inline-block;"><?= $safe($record['huyet_ap_tam_truong'] ?? '') ?></span> mmHg</div>
            <div class="vrow"><span class="vlabel">Nhịp thở:</span><span class="fill" style="border:0;border-bottom:1px dotted #000;"><?= $safe($record['nhip_tho'] ?? '') ?></span> lần/phút</div>
        </div>
    </div>

    <div class="sign">
        <div class="sign-box">
            <div>Ngày <?= $safe($record['ngay_ky'] ?? '') ?> tháng <?= $safe($record['thang_ky'] ?? '') ?> năm <?= $safe($record['nam_ky'] ?? '') ?></div>
        </div>
        <div class="sign-box">
